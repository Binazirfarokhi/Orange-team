...
export const uploadImage = async (type, id, multi) => {
    ...
    if (!result.canceled) {
        if (multi) {
            return Promise.all(result.assets.map(async asset => {
                ...
                const imgResult = await post(`/photos/64/${id}/${type}/${extension}`, { image64 });
                return imgResult.data.url;
            }));
        } else {
            const extension = result.assets[0].uri.substring(result.assets[0].uri.lastIndexOf('.'));
