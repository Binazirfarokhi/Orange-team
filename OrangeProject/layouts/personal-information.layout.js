...
function PersonalInformationScreen({ navigation }) {
    ...
    getPersistData("userInfo")
      .then(async (data) => {
        if (data && data.length > 0) {
          ...
          // setOrgPos()
          setOrgPos(
            bindOrgAndPosition(data[0].orgs, data[0].positionOfOrganization)
          );
        }
      })
  ...
  const save = async () => {
    ...
    if (org && org != null)
      positionOfOrganization = org.map((o) => orgPos[o] || POSITION_L1);
    const result = (
      await put(`/profile/personal/${emailAddress}`, {
        contactNumber,
        allowAddress,
        address,
        orgs: org,
        organization,
        positionOfOrganization,
      })
    ).data;
