    ...
    getPersistData("userInfo").then(async (data) => {
      if (data && data.length > 0) {
        ...
        if (childId) {
          ...
          const eventJoin = (
            await get(`/orgs/eventsJoined/${result.parent.id}`)
          ).data.data;
        ...
        <View style={styles.main}>
          <ImageBackground
            ...
            <ScrollView style={{ paddingRight: 20 }}>
              ...
              <CardWithNext
                onNext={() => navigation.navigate("AuthorizedTabs")}
                title={"Group Participation"}
                tile={`participating in ${events.length} group(s).`}
              />
              <CardWithNext
                onNext={() => navigation.navigate("AchivementDetail")}
                title={"Achievements"}
                tile={`Stats on goals and levels`}
              />
              {events.map((event) => (
