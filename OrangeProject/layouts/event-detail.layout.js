...
import { DATE_FORMAT_DISPLAY, DATE_FORMAT_PICKER, TIME_FORMAT_DISPLAY, TIME_FORMAT_PICKER, TYPE_PARENT, TYPE_VOLUNTEER } from "../util/constants";
...
const EventDetailScreen = ({ navigation, route }) => {
    ...
    const checkVolunteer = (id, email) => {
        navigation.navigate('VolunteerDetail',
            { email, fromOrg: true, orgId: organizationId, id: id })
    }

    useEffect(() => {
        get(`/orgs/event/${id}`).then(event => {
            const { eventName, date, time, organization, participantsList, images, location, eventType, ageGroup, participants, description, note, volunteers, participantsUserList } = event.data.data
            ...
            if (role === TYPE_VOLUNTEER && !joined) {
                setJoined(volunteers && volunteers !== null && volunteers.filter(vol => vol.id === currentUser.id).length > 0)
            }
                    ...
                    <ScrollView style={{ paddingRight: 20 }}>
                        ...
                        <View style={{ ...styles.item, display: 'flex', flexDirection: 'row' }}>
                            ...
                            <View style={{ display: 'flex' }}>
                                <Text style={styles.text1} onPress={() => navigation.navigate('OrganizationDetails', { id: organizationId })}> {organization}</Text>
                                <Text style={styles.text2}> Participants: {orgParticipants}</Text>
                            </View>
                        </View>
                        <View style={{ ...styles.item, display: 'flex', flexDirection: 'row' }}>
                        ...
                        <View style={styles.volunteers}>
                            {
                                volunteers.map(volunteer => (
                                    <TouchableOpacity onPress={() => checkVolunteer(volunteer.id, volunteer.signup.emailAddress)} key={volunteer.id}>
                        ...
                        {moment(date, DATE_FORMAT_DISPLAY).diff(new Date(), 'day') >= 0 &&
                            <Button onPress={joinEvent} disabled={joined}>{joined ? 'Joined' : 'Register'}</Button>
                        }
                    </ScrollView>
                </View>
            </View>
                            ...
                            <ScrollView>
                                {
                                    availableVolunteers
                                        ...
                                        .map(vol => (
                                            <View style={styles.volunteer} key={vol.id}><Text >{vol.signup.displayName}</Text></View>
                                        ))
                                }
                            </ScrollView>
                        </View>
