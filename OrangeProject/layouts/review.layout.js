...
function ReviewScreen({ navigation }) {
  ...
  const saveReview = async () => {
    ...
    const data = { userId: id, images, createdDt: new Date(), description, stars: selectedIndex + 1 };
      ...
      <KeyboardAvoidingView
        ...
        {/* <TouchableWithoutFeedback onPress={Keyboard.dismiss}> */}
          <View>
            ...
            <View>
              <ScrollView style={{ marginBottom: 250, paddingRight: 20 }} contentContainerStyle={{ flexGrow: 1 }}>
                <View>
                  <ButtonGroup
                  ...
                  <View style={{ minHeight: 100, flexDirection: 'row', flexWrap: 'wrap', display: 'flex', paddingTop: 30, paddingBottom: 30 }}>
                    {
                      loading && <ActivityIndicator size="large" />
                    }
                    {images.map((uri, i) => (
                      <View key={i} style={{ width: 100, height: 100 }}>
                        <FontAwesome name='trash' style={styles.trash} size={20} color={'red'} onPress={() => removeImage(uri)} />
                  ...
                  {reviews &&
                    reviews.map(review => (
                      <View key={review.id} style={styles.review}>
                        ...
                        <Text><Star count={review.stars} /></Text>
                        <Text>{review.description}</Text>
                        <View style={{ flexDirection: 'row', flexWrap: 'wrap', display: 'flex', paddingTop: 30, paddingBottom: 30, justifyContent: 'space-evenly' }}>
