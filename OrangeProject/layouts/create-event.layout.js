...
const CreateEventScreen  = ({navigation, route}) => {
    ...
    useEffect(()=> {
        if (id) {
            get(`/orgs/event/${id}`).then(data => {
                const { ageGroup, date, description, eventName, eventType, location, note, organization, participants, time, volunteers } = data.data.data;
    ...
    const save = () => {
        ...
        if(message !== '') {
            ...
            const data = {
                eventName, date, time, organization, location, organizationId, eventType, ageGroup, participants, description, note, volunteers: volunteers.map(vol=> vol.id)
            }
            if(id) {
            ...
            <View style={styles.main}>
                ...
                <View>
                    <ScrollView style={{marginBottom: 180, paddingRight: 20}}>
                        ...
                        {pickerOpen && 
                        <DateTimePicker
                            value={moment(date, DATE_FORMAT_DISPLAY).format(DATE_FORMAT_PICKER)}
                            mode='date'
